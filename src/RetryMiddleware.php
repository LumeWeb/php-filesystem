...
class RetryMiddleware {
	...
	private $defaultOptions = [
		...
		// Function to test if we should retry
		'maybe_retry_function_override'    => false,
	];
	 ...
	 */
	protected function shouldRetryHttpResponse( array $options, RequestInterface $request, ?ResponseInterface $response = null ): bool {
		...
		switch ( true ) {
			...
			default:
				...
				$retry = in_array( $statusCode, $statuses, true );
				if ( $retry && is_callable( $options['maybe_retry_function_override'] ) && false === $options['maybe_retry_function_override']( $request, $response ) ) {
					$retry = false;
				}
